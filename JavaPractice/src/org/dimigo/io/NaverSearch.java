	...
	public static void main(String[] args) {		
	    ...
	    try {
	        ...
	        for(int i=0;i<movieList.size();i++){
	        	System.out.println((1+i)+"."+movieList.get(i).getTitle()+" - "+movieList.get(i).getActors());
	        }
	        	
	        // files/movies.txt 파일에 write하기
	        File f1= new File("files/movies.txt");
	        try(BufferedWriter bw1 = new BufferedWriter(new FileWriter(f1,true));){
	        	for(int j=0;j<movieList.size();j++){
	        		bw1.write((1+j)+"."+movieList.get(j).getTitle()+" - "+movieList.get(j).getActors()+"\n");
	        	}bw1.flush();
	        }
	    } catch (IOException e) {
